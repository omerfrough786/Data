...
class UI {
    ...
    static addTransaction(id, des, amt) {
        let htmlItem = ''
        htmlItem = `
                <tr>
                <td>${des}</td>
...
class LS {
    ...
    static displayIncomes() {
       ...
       let htmlList = ''
        incomes.forEach(income => {
            ...
            htmlList += UI.addTransaction(income.id, income.des, income.amount)
        })
    ...
    static displayExpenses() {
        ...
        expenses.forEach(expense => {
            ...
            //Add Expense to UI
            htmlList += UI.addTransaction(expense.id, expense.des, expense.amount)
        })
...
function addTransactions() {
    ...
    id = id.toFixed()
    const transaction = new Transaction(id, description.value, amount.value)
    ...
    if (description.value === '') {
        ...
        listItem = UI.addTransaction(transaction.id, transaction.des, transaction.amount)
...
addIncToList.addEventListener('click', e =>{
    ...
    if(e.target.parentElement.classList.contains('delete-list')){
        ...
        console.log(tr)
        //Define id
        let id = Number(tr.children[3].innerText)
        console.log(id)
         //Remove from UI
